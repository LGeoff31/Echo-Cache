...
SocketClient::SocketClient(const std::string& ip, int port) {
    ...
    int connection = connect(this->sock_client,
...
void SocketClient::sendMessage(const std::string& message) {
    if (!this->connectionSucceeded) {
        std::cerr << "Tried to send message on failed connection" << std::endl;
        return;
    }
    send(this->sock_client, message.c_str(), message.length(), 0);
}
